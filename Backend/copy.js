    ...
    try {
        ...
        if (result) {
            ...
            const token = jwt.sign({ email: user.email },
                process.env.SECRET_KEY, { expiresIn: "4d" }
            )
...
app.post("/login", async (req, resp) => {
    try {
        ...
        const token = jwt.sign({ email: email }, process.env.SECRET_KEY, { expiresIn: "4d" })
...
// send messages --------------------------------
app.post("/send-message", verifyJWTToken, upload.single("media"), async (req, resp) => {
    ...
    if (!chatId || !text) {
        return resp.send({
            success: false,
            message: "ChatId and text both are required!"
        })
    }
...
// // ---- generate media url -------------------- 
app.post("/upload-media", verifyJWTToken, upload.single("media"), async (req, resp) => {
    if (!req.file) {
        return resp.status(400).send({
            success: false, msg: "File is not provided!"
        })
    }
